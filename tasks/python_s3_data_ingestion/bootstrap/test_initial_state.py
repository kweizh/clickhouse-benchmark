import os
import shutil
import subprocess
import pytest

PROJECT_DIR = "/home/user"

def test_python_installed():
    assert shutil.which("python3") is not None, "python3 binary not found in PATH."

def test_pip_installed():
    assert shutil.which("pip3") is not None, "pip3 binary not found in PATH."

def test_project_directory_exists():
    assert os.path.isdir(PROJECT_DIR), f"Project directory {PROJECT_DIR} does not exist."

def test_clickhouse_connect_installed():
    result = subprocess.run(["python3", "-c", "import clickhouse_connect"], capture_output=True)
    assert result.returncode == 0, "clickhouse_connect is not installed in the Python environment."
